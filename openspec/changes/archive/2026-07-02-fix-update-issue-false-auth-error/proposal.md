...
- `redmine-connection`: 修改「API 回應統一檢查」需求 — 非 JSON 檢查僅適用於帶 body 的回應;無 body 成功回應(HTTP 204,或 Content-Length 為 0)SHALL 視為成功,不得誤判為認證失敗

## Impact

- Affected specs: `redmine-connection`(MODIFIED「API 回應統一檢查」需求)
- Affected code:
